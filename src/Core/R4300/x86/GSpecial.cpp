...
void genjr()
{
    ...
    mov_reg32_preg32pimm32(EBX, EAX, (uintptr_t)(dst_block->block) + diff_need);
    cmp_reg32_imm32(EBX, 1);
    jne_rj(7);

    add_eax_imm32((uintptr_t)(dst_block->block) + diff_wrap); // 5
    jmp_reg32(EAX);                                          // 2

    mov_reg32_preg32pimm32(EAX, EAX, (uintptr_t)(dst_block->block) + diff);
...
void genjalr()
{
    ...
    free_all_registers();
    simplify_access();
    mov_eax_memoffs32((uintptr_t)dst->f.r.rs);
    mov_memoffs32_eax(&local_rs);
    ...
    if ((dst->addr + 4) & 0x80000000)
        mov_m32_imm32((void *)((dst - 1)->f.r.rd + 1), 0xFFFFFFFF);
    else
        mov_m32_imm32((void *)((dst - 1)->f.r.rd + 1), 0);
...
void gendmultu()
{
    ...
    mov_eax_memoffs32((uintptr_t)dst->f.r.rs);
    mul_m32((uintptr_t)dst->f.r.rt); // EDX:EAX = temp1
    ...
    mul_m32((uintptr_t)dst->f.r.rt); // EDX:EAX = temp3

    add_reg32_reg32(EBX, EAX);
    adc_reg32_imm32(ECX, 0); // ECX:EBX = result2
    mov_m32_reg32((char *)(&lo) + 4, EBX);
