...
static const char *s_vertexShaderSource = "#version 100\n"
...
static GLuint CompileShader(GLenum type, const char *source)
{
    ...
    if (shader == 0)
    {
        g_ef->log_error(L"GLSL combiner: glCreateShader failed");
        return 0;
    }
...
static std::string BuildFragmentSource(Combiner *color, Combiner *alpha)
{
    ...
    for (int cycle = 0; cycle < numCycles; cycle++)
    {
        ...
        if (!firstCycle)
        {
            body += "    combined = clamp(rgb, 0.0, 1.0);\n";
            body += "    combinedA = clamp(a, 0.0, 1.0);\n";
        }
    ...
    body += "    vec4 result = clamp(vec4(rgb, a), 0.0, 1.0);\n";
    ...
    source += "varying vec2 vTexCoord0;\n";
    source += "varying vec2 vTexCoord1;\n";
    source += "varying float vFog;\n";
...
{
    if (s_vertexShader == 0) s_vertexShader = CompileShader(GL_VERTEX_SHADER, s_vertexShaderSource);
...
void GLSLCombiner_Init()
{
    ...
    if (!GLEW_VERSION_2_0)
    {
        g_ef->log_error(L"GLSL combiner: OpenGL 2.0 not available, shaders will fail to compile");
        return;
    }
